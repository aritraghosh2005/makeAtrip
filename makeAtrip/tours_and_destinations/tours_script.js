...
// --- REDIRECT TO STAYS PAGE (WITH HREF FALLBACK) ---
function redirectToStays(event) {
    const params = new URLSearchParams(window.location.search);
    const selectedCity = params.get('city');
    
    if (selectedCity) {
        // Stop the normal href from taking us to the main index page
        event.preventDefault(); 
        // Redirect to the specific city page instead
        window.location.href = `../hotels/city_stays.html?city=${encodeURIComponent(selectedCity)}`;
    }
    // If there is no city selected, the script does nothing, 
    // and the button's normal href="../hotels/index.html" takes over naturally!
}